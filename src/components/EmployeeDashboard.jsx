...
import { getGoals, saveGoals } from '../data';
import { Plus, Save, Send, Edit, Target, Clock, AlertTriangle } from 'lucide-react';
  ...
  return (
    <div className="animate-fade-in delay-100">
      ...
      {goals.length === 0 ? (
        ...
        <div className="grid">
          {goals.map((goal, idx) => (
            <div key={goal.id} className={`glass-panel animate-fade-in delay-${(idx % 3 + 1) * 100}`} style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              {canEdit && (goal.status === 'Draft' || goal.status === 'Returned') ? (
                <>
                  ...
                  <div className="grid grid-cols-2">
                    <select className="form-select" value={goal.thrustArea} onChange={e => handleGoalChange(goal.id, 'thrustArea', e.target.value)}>
                      <option value="Business Growth">Business Growth</option>
                      <option value="Operational Excellence">Operational Excellence</option>
                      <option value="Customer Success">Customer Success</option>
                      <option value="Innovation">Innovation</option>
                    </select>
                    <select className="form-select" value={goal.uom} onChange={e => handleGoalChange(goal.id, 'uom', e.target.value)}>
                ...
                <>
                  <h3 style={{ fontSize: '1.25rem', marginTop: '0.5rem' }}>{goal.title || 'Untitled Goal'}</h3>
